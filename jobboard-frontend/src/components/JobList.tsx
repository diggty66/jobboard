...
export default function JobList({ jobs, className = "" }: Props) {
  ...
  return (
  <main className={`overflow-y-auto bg-gray-100 p-6 dark:bg-gray-900 transition-colors duration-500 ${className}`}>
    {jobs.length === 0 ? (
          ...
          {jobs.map((j) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-gray-700 dark:text-gray-300">
                💰 {j.salary.toLocaleString()} {j.currency}
              </p>
              {user?.role === "seeker" && (
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-2">
              {selected.region}, {selected.country} — {selected.visa_type}
            </p>
            <p className="text-gray-800 dark:text-gray-300 mb-2">
              💰 {selected.salary.toLocaleString()} {selected.currency}
            </p>
